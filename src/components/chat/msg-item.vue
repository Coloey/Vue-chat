<template>
  <li>
    <router-link
      :to="{
        path: '/chat/dialogue',
        query: { mid: item.mid, name: item.user.name },
      }"
      v-on:click="toggleMsgRead(item.mid,'enter')"
    >
      <div class="list-info">
        <div class="header-box">
          <!-- 未读且未屏蔽，显示新信息数量 -->
          <i class="new-msg-count" v-show="!read && !quiet">{{
            item.msg.length
          }}</i>
          <!-- 未读且屏蔽，只显示小红点 -->
        ...
        <div class="desc-box">
          <div class="desc-time">{{ time(item.msg[item.msg.length - 1].date) }}</div>
